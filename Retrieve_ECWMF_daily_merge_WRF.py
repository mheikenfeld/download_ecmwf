...
start_date=datetime.date(2013,5,12)
end_date=datetime.date(2013,5,31)
...
# check if output directory year/month exists otherwise create
    ...
    target_sfc= "%sERA-Interim_sfc_%s.grib"%(monthpath,datestr) 
    target_pl="%sERA-Interim_pl_%s.grib"%(monthpath,datestr) 
    target_wrf="%sERA-Interim_wrfinput_%s.grib"%(monthpath,datestr)
    print("######### ERA-interim  #########")
